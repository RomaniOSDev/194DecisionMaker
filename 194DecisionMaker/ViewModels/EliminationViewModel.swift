...
final class EliminationViewModel: ObservableObject {
    ...
    private func startTournament() {
        matches = bracketEngine.buildInitialMatches(from: allOptions)
        autoResolveByes()
        checkRoundComplete()
    }
    ...
    private func checkRoundComplete() {
        ...
        if winners.count == 1 {
            champion = winners[0]
            finish(with: winners[0])
            return
        }

        let next = bracketEngine.advanceWinners(from: matches, round: currentRound)
        matches.append(contentsOf: next)
        currentRound += 1
        autoResolveByes()
    }

    private func finish(with option: Option) {
